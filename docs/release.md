...
- Root package: `@loongphy/codex-auth-proxy`
- Platform packages:
...
- The root package exposes the `codex-auth-proxy` command and depends on the platform packages through `optionalDependencies`.
...
- Windows builds include both `codex-auth-proxy.exe` and `codex-auth-proxy-auto.exe`; the helper is used only by the managed auto-switch task.
...
- The root CLI wrapper uses that field so `codex-auth-proxy --version` prints `codex-auth-proxy <version> (preview pr-6 b6bfcf5)` for preview installs only.
...
- npm provenance validation requires the package `repository.url` metadata to match the GitHub repository URL exactly: `https://github.com/Loongphy/codex-auth-proxy`
